...
router.post("/login_masyarakat", loginLimiter, async (req, res) => {
  try {
    ...
    if (!identifier_masyarakat || !password_masyarakat) {
      return res
        .status(400)
        .json({ message: "Harap masukkan username/NIK dan password" });
    }
    const user = await masyarakat
      .findOne({
        $or: [
          { username_masyarakat: identifier_masyarakat },
          { nik_masyarakat: identifier_masyarakat },
        ],
      })
      .lean();
    ...
    if (user.verifikasi_akun_masyarakat !== "diterima") {
      if (user.verifikasi_akun_masyarakat === "pending")
        return res.status(403).json({ message: "Akun belum diverifikasi" });
      if (user.verifikasi_akun_masyarakat === "ditolak")
        return res.status(403).json({ message: "Akun anda ditolak" });
...
router.post("/login_superadmin", async (req, res) => {
  try {
    const { username_superadmin, password_superadmin, encrypted } = req.body;
    ...
    let decryptedPassword = password_superadmin;
    if (encrypted) {
      try {
    ...
    // Compare decrypted password with bcrypt hash
    const isMatch = await bcrypt.compare(
      decryptedPassword,
